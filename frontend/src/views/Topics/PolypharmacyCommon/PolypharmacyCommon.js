...
import {PolypharmacyCommonBackendUpdate,submitData} from "./PolypharmacyCommonBackend";
...
export default function PolypharmacyCommon() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:8887/api/polypharmacycommon')
  ...
  const update_value = (event) => {
    ...
    if (admin) {
      ...
      if (event.target.value !== value) {
        ...
        PolypharmacyCommonBackendUpdate(event.target.name, event.target.id, event.target.value).then((data) => {
  ...
  const handleDelete = async (Description) =>{
    ...
    try{
      ...
      await axios.delete('http://localhost:8887/api/PolypharmacyCommon/delete/'+Description);
  ...
  if (data.length > 0) {
    ...
    if (admin) {
        ...
        <Box
            ...
            <Typography variant="h3" id="topicHeader">COMMON DDIs WITH PSYCHOTROPICS</Typography>
            </Box>
